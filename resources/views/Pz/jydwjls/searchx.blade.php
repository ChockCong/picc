...
<div class="panel panel-default">
    <div class="panel-heading">未审核工单查询</div>
    <div class="panel-body" style="font-size:110%">
        <form method="post" action="{{url('jydwjlsconsearch')}}">
    ...
    <div class="panel-heading">未审表单</div>
            ...
            @if($row!=null)
                @foreach($row as $val)
                    ...
                    <tr>
                        ...
                        <td>{{$val->State}}</td>
                        <td>{{$j}}</td>
                        <td><a href="{{url('jydwjlsdetailx/'.$val->Order1)}}" target="Center">查看</a></td>
                    </tr>
                @endforeach
            @endif
            </tbody>
